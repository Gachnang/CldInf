...
\lstinputlisting[language=bash,caption={/etc/netplan/50-cloud-init.yaml on R1}]{configs/R1/netplan/50-cloud-init.yaml}
...
\lstinputlisting[language=bash,caption={/etc/bird/bird.conf on R1}]{configs/R1/bird/bird.conf}
...
\lstinputlisting[language=bash,caption={/etc/nftables.conf on R5}]{configs/R5/nftables.conf}
...
\begin{figure}[H]
	\begin{center}
		...
		\label{fig:client-OSPF-Capture-ens2}
	\end{center}
\end{figure}
...
\begin{figure}[H]
	\begin{center}
		...
		\label{fig:client-OSPF-Capture-ens3}
	\end{center}
\end{figure}
...
Wir geben folgenden Befehl auf R3 bei ens4 ein damit wir 10\% Packetlossrate haben:\\
tc qdisc add dev ens4 root netem loss 10\% 
...
Nachdem wir die oben genannte Regel mit del statt add als Befehl wieder entfernt haben, wollten wir noch sehen wie es mit 10s delay und 20ms jitter aussieht. Mit folgendem Befehl haben wir die Regel bei R3 ens4 eingestellt:\\
tc qdisc add dev ens4 root netem delay 10ms 20ms
...
\lstinputlisting[language=python,caption={injection.py on MITM}]{configs/MITM/injection.py}

Danach kann der Proxy auf MITM gestartet werden mit dem Befehl \lstinline!mitmproxy -s injection.py!.
...
Vom Client aus rufen wir den Server erneut auf, diesmal jedoch über den Proxy: \lstinline!curl --proxy 10.0.100.2:8080 192.168.1.100:8080!. (oder setzen \lstinline!export http_proxy=10.0.100.2:8080!.)

\medskip

Eigentlich sollte nun die definierte Antwort kommen ''<h1>Injected</h1>...'', jedoch haben wir dies nicht hinbekommen.
Jedoch sehen wir die Verbindung, und da die Firewall eine Verbindung vom MITM aus nicht erlaubt, den time-out:

\begin{figure}[H]
  \begin{center}
  	\includegraphics[width=0.90\textwidth]{"images/MITM_Req_timeout"}
  	\caption{MITM Timeout}
  	\label{fig:MITM_Req_timeout}
  \end{center}
\end{figure}

\begin{figure}[H]
  \begin{center}
  	\includegraphics[width=0.90\textwidth]{"images/MITM_Answer_1"}
  	\caption{Client Timeout}
  	\label{fig:MITM_Answer_1}
  \end{center}
\end{figure}
...
Alle erarbeiteten Configs sind im Ordner ''configs'' neben dieser PDF-Datei zu finden. Jeder Container hat darin einen eigenen Unterordner. Der Script welche alle Configs generiert ist dort ebenfalls zu finden.
